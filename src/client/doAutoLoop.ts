...
export default async function doAutoLoop(client: HealthScreeningBotClient, logChannel: TextChannel) {
    ...
    for (const config of (await sequelize.query(
        ...
        batchTimes.set([config.timeHours, config.timeMinutes], batchTimes.get([config.timeHours, config.timeMinutes]) ? batchTimes.get([config.timeHours, config.timeMinutes]) + 1 : 1);
        await client.screeningClient.queueDailyAuto(await client.users.fetch(config.userId), {
