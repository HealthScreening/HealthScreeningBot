...
import {generateScreenshot, GenerateScreenshotParams} from "./utils/produce_screenshot";
...
export class ScreeningClient {
    ...
    private static async getUserInfo(options: {
        ...
        const configItem = await Config.findOne({where: {userId: options.userId}})
        if (configItem === null) {
    ...
    private processCooldowns(userId: string, sendMessageOptions: MessageOptions): boolean {
        if (this.cooldowns.has(userId)) {
            const messageOptions = {
                content: "You are on cooldown! Please wait a minute before using this command again.",
                ephemeral: true,
                ...sendMessageOptions,
            }
    ...
    private async dealWithQueue(params: ProcessParams) {
        if (this.queue.willQueue()) {
            const messageOptions: MessageOptions = {
                content: "Your request has been queued! You will be notified when the screening is ready. You are currently at position " + this.queue.determineNextPosition(1) + " in the queue.",
                ...params.multiMessageParams,
            }
            sendMessage(messageOptions);
        } else {
    ...
    public async queueAutoCommand(userId: string, multiMessageParams: MessageOptions): Promise<void> {
    ...
    public async queueDailyAuto(user: User, auto: AutoBatchOptions & { manual?: boolean }) {
        ...
        let content = `If you enjoyed the bot, please share this server with your friends!: https://discord.gg/yJbvcD4QBP\n----\nHere is the screenshot that has been auto-generated for you:`;
        if (auto.manual) {
